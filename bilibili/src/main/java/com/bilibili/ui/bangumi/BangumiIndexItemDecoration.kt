...
class BangumiIndexItemDecoration(private val spanSizeLookup: GridLayoutManager.SpanSizeLookup) : RecyclerView.ItemDecoration() {
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State?) {
        ...
        if (spanSizeLookup.getSpanSize(position) == 3) {
            //追翻页列表前两项为静态item
            if (position > 1) {
                if (view.tag !is BangumiDividerBinder.BangumiDivider) {
